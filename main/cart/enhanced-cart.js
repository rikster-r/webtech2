...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const cartManager = {
    items: [
      { id: 1, name: 'Premium Wireless Headphones', price: 13980, quantity: 1, emoji: '🎧', color: 'Black' },
      { id: 2, name: 'Smartphone Case', price: 2490, quantity: 2, emoji: '📱', color: 'Clear, Size: iPhone 15' },
      { id: 3, name: 'Smart Watch Band', price: 4950, quantity: 1, emoji: '⌚', color: 'Black, Size: 42mm' }
    ],
      ...
      cartItem.innerHTML = `
        <div class="d-flex align-items-center gap-3">
          <div class="bg-light rounded-3 d-flex align-items-center justify-content-center item-emoji" 
               style="width:80px; height:80px; font-size:24px;">${item.emoji}</div>
          <div>
        ...
        <button class="btn btn-link text-danger fs-5 remove-item" data-item-id="${item.id}">×</button>
      `;
  ...
  function updateOrderSummary() {
    ...
    // Update subtotal
    const subtotalElement = summaryContainer.querySelector('.d-flex.justify-content-between.border-bottom.py-2');
    if (subtotalElement) {
    ...
    // Update shipping
    const shippingElements = summaryContainer.querySelectorAll('.d-flex.justify-content-between.border-bottom.py-2');
    ...
    // Update total
    const totalElements = summaryContainer.querySelectorAll('.d-flex.justify-content-between.pt-3');
    if (totalElements[0]) {
    ...
    if (checkoutButton) {
      checkoutButton.addEventListener('click', (e) => {
        ...
        if (window.AdvancedFeatures) {
          ...
          window.AdvancedFeatures.AnimationManager.bounce(checkoutButton, 1.1, 300);
          window.AdvancedFeatures.showNotification('Redirecting to checkout...', 'success');
        }
    ...
    if (removeButton) {
      removeButton.addEventListener('click', () => {
        if (cartManager.items.length === 0) {
          if (window.AdvancedFeatures) {
            window.AdvancedFeatures.showNotification('Cart is already empty!', 'warning');
          }
          return;
        }
        ...
        if (window.AdvancedFeatures) {
          window.AdvancedFeatures.SoundManager.playSound('remove');
          window.AdvancedFeatures.showNotification('All items removed from cart!', 'success');
        }
  ...
  function createInteractiveFeatures() {
    ...
    const saveForLaterContainerElement = document.getElementById('saveForLaterItems');
    ...
    document.addEventListener('click', (e) => {
      if (e.target.classList.contains('add-back-btn')) {
        ...
        const itemPrice = parseInt(itemElement.querySelector('.text-muted').textContent.replace(/[^\d]/g, ''));
        ...
        if (window.AdvancedFeatures) {
          window.AdvancedFeatures.SoundManager.playSound('add');
          window.AdvancedFeatures.showNotification(`${itemName} added back to cart!`, 'success');
        }
